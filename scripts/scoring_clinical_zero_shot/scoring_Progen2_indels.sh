...
NUM_DATASETS=$(($(wc -l < $clinical_reference_file_path_indels) - 1))
...
for worker_id in "${!GPUS[@]}"; do
    ...
    run_worker "$worker_id" "$gpu_id" > "../../logs/progen2_small_base/clinical_zero_shot_indels_03_08/gpu_${gpu_id}.log" 2>&1 &
done
